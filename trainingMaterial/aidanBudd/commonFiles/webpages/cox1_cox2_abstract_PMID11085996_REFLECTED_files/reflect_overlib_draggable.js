...
////////
if (typeof olInfo == 'undefined' || typeof olInfo.meets == 'undefined' || !olInfo.meets(4.23)) alert('overLIB 4.23 or later is required for the Draggable Plugin.');
else {
...
function moveEl(e) {
  //09-11-2008 heiko
  //injected for hiding div
  if(document.getElementById("overLayDiv")){
    document.getElementById("overLayDiv").style.display = "block";
  }
  //stop heiko
    //console.info("moveEL("+e.pageX+","+e.pageY+")");
    var e=(e) ? e : event;
    var dX,dY,X,Y;
    //  get new mouse location
    X=(e.pageX || eval('e.clientX+o3_frame.'+docRoot+'.scrollLeft'));
    Y=(e.pageY || eval('e.clientY+o3_frame.'+docRoot+'.scrollTop'));
    //2008-04-08,nige: chkCursorPosition() causes popup motion to stop when
    //cursor leaves popup titlebar. This happens for olDragObj but not for
    //olImgObj. In fact chkCursorPosition() always returns true for any
    //non-object first argument, otherwise it tests the relative location of the
    //popup to something(?) and returns false if outside some bounds. I can't
    //see what this test is for anyway: removed.
    //
    //2008-09-04,Heiko Evangelos: put back the clause,
    //chkCursorPosition()
    //multiplied dx/dy with 5 to increase speed
//    if (chkCursorPosition(o3_dragcap ? olDragObj : olImgObj,X,Y)){
    	if (olNs4) {
    		dX=X-cX; cX=X;
    		dY=Y-cY; cY=Y;
